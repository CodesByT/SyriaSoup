...
import {
  ...
  translateModel,
  translateCarField,
} from "../utils/translation-helpers"; // Keep existing translation helpers
import { locationOptionsData } from "../utils/constants"; // Import location options data
...
export default function CarCard({
  ...
  // Get translated values for make, model, transmission, fuelType, status
  // These will continue to use your existing translation-helpers or t() directly
  ...
  const formatPrice = (price: string | undefined): string => {
    ...
    return `$${parseInt(price).toLocaleString()}`; // Ensure price is parsed to number for toLocaleString
  };
  ...
  const getStatusText = (status: string) => {
    // This logic can be simplified if your i18n setup handles the status keys directly.
    // However, sticking to your provided structure for this part.
    if (isArabic) {
      switch (status) {
        case "approved":
          return "موافق عليه";
    ...
    // If not Arabic, then use the `t` function to translate.
    // Ensure you have these keys in your i18n `en` bundle.
    return t(status);
  };
        ...
        <View style={styles.content}>
          ...
          <View style={styles.detailsContainer}>
            ...
            >
              ...
              >
                {car.kilometer
                  ? `${parseInt(car.kilometer).toLocaleString()} km` // Parse to int for toLocaleString
                  : t("notSpecified")}
              </Text>
            </View>
            ...
            >
              ...
              >
                {getLocalizedLocation(car.location)} {/* Use new helper here */}
              </Text>
            </View>
          </View>
